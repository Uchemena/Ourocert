  ...
  try {
    ...
    for (const cert of certificates || []) {
      try {
        ...
        if (batch.template.type === 'uploaded') {
          // For uploaded templates: composite text onto image
          fileUrl = await generateUploadedCertificate(supabase, batch.template, cert)
        } else {
          // For designed templates: render HTML to image/PDF
          fileUrl = await generateDesignedCertificate(supabase, batch.template, cert)
        }

        // Update certificate with file URL and status
        await supabase
          .from('certificates')
          .update({ 
            file_url: fileUrl,
        ...
        await fetch(`${Deno.env.get('SUPABASE_URL')}/functions/v1/send-certificates`, {
          method: 'POST',
          headers: {
...
async function generateUploadedCertificate(supabase: any, template: any, cert: any): Promise<string> {
  ...
  for (const field of fields) {
    ...
    const text = recipientData[field.name] || field.name
